  ...
  return (
    <div className="ai-page">
      ...
      <section className="ai-composer">
        <div className="ai-composer-header"><div className="ai-avatar"><WandSparkles size={18} /></div><div><strong>New transaction</strong><span>Include amount, payment name, and bank or card</span></div></div>
        <textarea value={prompt} onChange={(event) => { setPrompt(event.target.value); setError(""); setMessage(""); }} onKeyDown={(event) => { if ((event.ctrlKey || event.metaKey) && event.key === "Enter") understandPrompt(); }} placeholder="e.g. Spent 450 at Amazon using HDFC Bank" rows={4} aria-label="Describe your transaction" />
      ...
      <div className="ai-examples"><span>Try an example</span>{examples.map((example) => <button key={example} onClick={() => setPrompt(example)}>{example}</button>)}</div>
